...
function updateStatusLines() {
  ...
  console.log("🔄 Status lines updated:", { total, selected });
}
...
function enterEditMode(id) {
  ...
  if (typeof g.assigner !== "object" || !g.assigner) g.assigner = { name:"", phone:"", email:"" };
  if (typeof g.payer !== "object" || !g.payer) g.payer = { name:"", phone:"", email:"" };
...
function buildFieldValuesFromGame(g) {
  if (!g.assigner || typeof g.assigner !== "object") g.assigner = { name:"", phone:"", email:"" };
  if (!g.payer || typeof g.payer !== "object") g.payer = { name:"", phone:"", email:"" };
...
async function fillTemplatePdfForGame(templateBytes, tpl, g) {
  ...
  fields.forEach(f => { try { f.setText(""); } catch (e) {} });

  const vals = buildFieldValuesFromGame(g);
  for (const [k, v] of Object.entries(vals)) {
    if (fieldNames.includes(k)) {
      try { form.getTextField(k).setText(v || ""); } catch (e) {}
    }
  }
...
async function generateSinglePdf(g) {
  ...
  saveAs(new Blob([finalBytes], { type: "application/pdf" }), `Game-${g.game_number || "Match"}.pdf`);
}
...
async function generateCombinedPdf() {
  ...
  try {
    ...
    const TOP    = { x: 72, y: 432 };
    const BOTTOM = { x: 72, y: 72 };
    ...
    saveAs(new Blob([finalBytes], { type: "application/pdf" }), "MatchCards.pdf");
...
updateSelectedCountUI()
updateStatusLines()
;     // Optional: refresh game list table
